...
const storyData = [
  {
    ...
    text: "It wasn’t just a meeting, it was a collision of worlds. The air felt lighter, the coffee tasted sweeter, and for the first time, everything made sense.",
  },
  {
    id: 2,
    img: img2,
    ...
    text: "Under the glow of the holiday lights, we shared a moment that felt like time had stopped, and everything else faded away.",
  },
  {
    id: 3,
    img: img3,
    ...
    text: "From quiet mornings to beautiful journeys, every second has been a memory worth holding forever.",
  },
];
...
export default function Cards() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 🚀 PREMIUM CARD ANIMATION (batch = smooth)
      ...
      // 🌿 SVG ENTRY ONLY (no floating = no lag)
  ...
  return (
    <section
      ref={containerRef}
      className="relative py-20 px-5 sm:px-10 overflow-hidden"
    >
      ...
      <div className="max-w-5xl mx-auto space-y-24">
        {storyData.map((item, i) => (
          <div
            key={item.id}
            className="card opacity-0 translate-y-10 grid md:grid-cols-2 gap-8 items-center"
          >
            {/* IMAGE */}
            <div
            ...
            >
              ...
              {/* DATE */}
              <span className="absolute bottom-2 bg-white text-[10px] px-3 py-1 rounded shadow">
                {item.date}
              </span>
            </div>
            ...
            >
              <h2 className="text-xl sm:text-2xl font-medium mb-2 text-gray-800">
                {item.title}
              </h2>
